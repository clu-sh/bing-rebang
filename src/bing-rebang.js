...
function fetchKeywordsBySource(){
    ...
    if(keywords){
        renderKeywordsBySource(sessionStorage.getItem(cacheKey)?JSON.parse(keywords):null);
...
function renderRebang(){
    ...
    if($('#rebang').length==0 && $('#b_content').length>0){
        ...
        if(sessionStorage.getItem("RebangChannels")!==null){
            initChannels(JSON.parse(sessionStorage.getItem("RebangChannels")),getCurrentChannel());
...
function execAutoSearch(lockKey) {
    ...
    if(jobLockExpires.length ==0 || ( jobLockExpires.length >0 && new Date(jobLockExpires) < new Date())){
        // 生成随机延迟时间
        ...
        let currentKeywordIndex= Number(localStorage.getItem('currentKeywordIndex')??0);
